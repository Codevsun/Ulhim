export default function UserIntroduction() {
  const journeySteps = [
    {
      id: 1,
      title: "Inspire",
      description: "Discover new possibilities and ignite your passion for learning through innovative approaches.",
      color: "purple",
      position: "left",
      icon: (
        <path
    ...
    },
    {
      id: 3,
      title: "Growth",
      description: "Experience continuous development through structured learning paths and expert guidance.",
      color: "green",
      position: "left",
      icon: (
        <path
  ...
  return (
    <section id="user-introduction" className="relative min-h-screen w-full bg-gradient-to-b from-[#0a0611] to-[#0a0915] py-20 text-white">
            ...
            <div key={step.id} className="mb-24 flex items-center justify-between">
              {step.position === "left" ? (
                <div className={`animate-slide-right w-5/12 text-right`}>
                  <h3 className={`mb-2 text-2xl font-bold text-${step.color}-400`}>{step.title}</h3>
              ...
              <div className={`relative flex h-16 w-16 animate-pulse items-center justify-center rounded-full border border-${step.color}-500/20 bg-${step.color}-500/10`}>
                <svg
                  className={`h-8 w-8 text-${step.color}-400`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
